...
introparagraph: This project implements a circuit planner using a tree. This project was a little hard
as it took many iterations to keep our code from being redundant. For example, for the swap_topology function
we figured out a way to make it more efficient when we realized that the module pointer is a null value going 
into the function, so the only if statement we added was to check if it was actually a cutline. 

Is leaf node and internal node check if the node is a leaf or internal node. Rotate and recut alter the values of the node,
and get total resources gets the total resources used during recursion. Postfix traversal searches the tree and adds nodes
in the arrays. Finally, init slicing tree initializes the tree using recursion.*/
...
int is_in_subtree(node_t* a, node_t* b) {
  ...
  if (is_in_subtree (a -> right, b)) {
    return 1;
  } 
  //if (is_in_subtree (a , b->parent)) {
    //return 1;
  //}
  // TODO: (remember to modify the return value appropriately)
  return 0;
...
void swap_module(node_t* a, node_t* b) {
  if(is_leaf_node(a) == 0 || is_leaf_node(b) == 0) return;
...
int get_total_resource(node_t* ptr)
{
  ...
  if(!is_leaf_node(ptr)){
    sum = sum + get_total_resource(ptr->right) + get_total_resource(ptr->left);
  }else{
...
node_t* init_slicing_tree(node_t* par, int n) {
  ...
  node_t * nnode = (node_t *) malloc(sizeof(node_t));
    if(n == num_modules-1){
      ...
      nnode->left = NULL;
      nnode->parent = par;
      nnode->cutline = UNDEFINED_CUTLINE;
      nnode->module = &modules[n];
      return nnode;
    }

    node_t* rnode = (node_t *) malloc(sizeof(node_t));
    rnode->left = NULL;
    rnode->parent = nnode;
    rnode->right = NULL;
    rnode->cutline = UNDEFINED_CUTLINE;
    rnode->module = &modules[n];

    nnode->cutline = V;
    nnode->module = NULL;
    nnode->parent = par;
    nnode->left = init_slicing_tree(par, n+1);
    nnode->right = rnode;
